import { Container, Image, Grid, Group, Paper, Title, UnstyledButton, Stack, Text, Badge, Avatar, Divider, Box, Code, ScrollArea } from "@mantine/core";
...
const CrashReportView = () => {
  ...
  if(isSuccess && !isLoading && !report?.body) return <ErrorElement>No report found</ErrorElement>
  ...
  return (
    <Container fluid>
      <Group ref={ref}>
        <UnstyledButton component={Link} onClick={() => navigate(-1)} p={0} m={0}>
          <ArrowLeft />
        </UnstyledButton>
      ...
      <Grid >
        ...
        <Grid.Col span={{ base: 12, md: 4 }}>
          <Paper style={{viewTransitionName: 'crsh-report-view'}} component={ScrollArea} h={`calc(100vh - ${height + 95}px)`} w={'100%'} p={10} m={0}>
            ...
            <Group px={5} py={10}>
              ...
              <Badge color={severityColors[report?.body?.severity_level]} size="xs">{report?.body?.severity_level}</Badge>
            </Group>
            ...
            <CrashReportTimelineCard
              margin={10}
              padding={0}
              severity={report?.body?.severity_level} errorDetails={{
